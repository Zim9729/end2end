...
{
	const nvinfer1::Dims input_dims = this->engine->getTensorShape(INPUT);
	this->in_size = get_size_by_dims(input_dims);
	CHECK(cudaMalloc(&this->buffs[0], this->in_size * sizeof(float)));

	this->context->setInputShape(INPUT, input_dims);
	const char* output_names[NUM_OUTPUT] = { NUM_DETS, BBOXES, SCORES, LABELS };
	for (int i = 0; i < NUM_OUTPUT; i++)
	{
		const nvinfer1::Dims output_dims = this->context->getTensorShape(output_names[i]);
		this->out_sizes[i].first = get_size_by_dims(output_dims);
		this->out_sizes[i].second = DataTypeToSize(this->engine->getTensorDataType(output_names[i]));
	}
	...
	this->context->setTensorAddress(INPUT, this->buffs[0]);
	this->context->setTensorAddress(NUM_DETS, this->buffs[NUM_INPUT + 0]);
	this->context->setTensorAddress(BBOXES, this->buffs[NUM_INPUT + 1]);
	this->context->setTensorAddress(SCORES, this->buffs[NUM_INPUT + 2]);
	this->context->setTensorAddress(LABELS, this->buffs[NUM_INPUT + 3]);
	if (warmup)
	{
		for (int i = 0; i < 10; i++)
		{
